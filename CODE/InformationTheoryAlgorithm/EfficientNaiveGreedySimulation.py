...
def pattern_matrix(words1, words2):
    ...
    for i, j in product(range(5), range(5)): 
        #Iterate over all combinations of letters in the cartesian product of (0:5)x(0:5)
        equalities[:, :, i, j] = np.equal.outer(list1[:, i], list2[:, j])

    matrix = np.zeros((l1,l2), dtype=np.uint8)

    for i in range(5):
        #Green pass - matches[a, b] : words1[a][i] == words2[b][i] => equalities[a,b,i,i] for all (a,b)
